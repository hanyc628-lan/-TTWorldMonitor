...
export function simulateMatch(playerA: Player, playerB: Player, bestOf: BestOf = 5): MatchSimResult {
  const eloA = pointsToElo(playerA.points, playerA.rank);
  const eloB = pointsToElo(playerB.points, playerB.rank);
  const priorWinProbA = expectedScore(eloA, eloB);

  // 局胜率略向 0.5 收缩，体现单局波动
  const pGameA = 0.5 + (priorWinProbA - 0.5) * 0.85;
  // 分胜率再平滑一层
  ...
  return {
    ...
    note: '模拟基于 topspin-lab Elo 期望分 + 局分 Monte Carlo，非官方赛果',
  };
}
